...
Xの画面の背景に好きな画像を設定できる機能です。
ポップアップから画像をアップロードするだけで、Xの背景がリアルタイムに切り替わります。

- **画像アップロード** — ローカルの画像（最大5MB, JPG/PNG）を背景に設定
- **リアルタイム反映** — スライダーで透過率（0〜100%）とぼかし（0〜20px）を直感的に調整可能
- **ワンクリックでON/OFF・削除** — いつでも元の背景に戻せます
